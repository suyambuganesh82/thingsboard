...
public interface TenantRepository extends JpaRepository<TenantEntity, UUID> {
    ...
    @Query("SELECT t FROM TenantEntity t WHERE (:textSearch IS NULL OR ilike(t.title, CONCAT('%', CAST(:textSearch AS string), '%')) = true)")
    ...
    @Query("SELECT new org.thingsboard.server.dao.model.sql.TenantInfoEntity(t, p.name) " +
            "FROM TenantEntity t " +
            "LEFT JOIN TenantProfileEntity p on p.id = t.tenantProfileId " +
            "WHERE (:textSearch IS NULL OR ilike(t.title, CONCAT('%', CAST(:textSearch AS string), '%')) = true)")
