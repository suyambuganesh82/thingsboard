...
public interface DeviceRepository extends JpaRepository<DeviceEntity, UUID>, ExportableEntityRepository<DeviceEntity> {
    ...
    @Query("SELECT d FROM DeviceEntity d WHERE d.tenantId = :tenantId " +
            ...
            "AND (:textSearch IS NULL OR ilike(d.name, CONCAT('%', CAST(:textSearch AS string), '%')) = true " +
            "OR ilike(d.label, CONCAT('%', CAST(:textSearch AS string), '%')) = true)")
...
//            "AND (:textSearch IS NULL OR ilike(d.name, CONCAT('%', CAST(:textSearch AS string), '%')) = true " +
//            "OR ilike(d.label, CONCAT('%', CAST(:textSearch AS string), '%')) = true " +
//            "OR ilike(d.type, CONCAT('%', CAST(:textSearch AS string), '%')) = true " +
//            "OR ilike(d.customerTitle, CONCAT('%', CAST(:textSearch AS string), '%')) = true)"
//    )

    @Query("""
            SELECT d FROM DeviceInfoEntity d WHERE d.tenantId = :tenantId AND (CAST(:customerId AS java.util.UUID) IS NULL OR d.customerId = :customerId)
            AND (CAST(:edgeId AS java.util.UUID) IS NULL OR d.id IN (SELECT re.toId FROM RelationEntity re WHERE re.toType = 'DEVICE' AND re.relationTypeGroup = 'EDGE' AND re.relationType = 'Contains' AND re.fromType = 'EDGE' AND re.fromId = :edgeId))
            AND (:deviceType IS NULL OR d.type = :deviceType)
            AND (CAST(:deviceProfileId AS java.util.UUID) IS NULL OR d.deviceProfileId = :deviceProfileId)
            AND (:filterByActive = FALSE OR d.active = :deviceActive)
            AND (CAST(:textSearch AS string ) IS NULL OR ilike(d.name, CONCAT('%', CAST(:textSearch AS string ), '%')) = true
            OR ilike(d.label, CONCAT('%', CAST(:textSearch AS string ), '%')) = true
            OR ilike(d.type, CONCAT('%', CAST(:textSearch AS string ), '%')) = true
            OR ilike(d.customerTitle, CONCAT('%', CAST(:textSearch AS string ), '%')) = true)
           """
