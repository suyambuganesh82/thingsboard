...
public interface EntityViewRepository extends JpaRepository<EntityViewEntity, UUID>, ExportableEntityRepository<EntityViewEntity> {
    ...
    @Query("SELECT e FROM EntityViewEntity e WHERE e.tenantId = :tenantId " +
            "AND (:textSearch IS NULL OR ilike(e.name, CONCAT('%', CAST(:textSearch AS string), '%')) = true)")
    ...
    @Query("SELECT e FROM EntityViewEntity e WHERE e.tenantId = :tenantId " +
            "AND e.type = :type " +
            "AND (CAST(:textSearch AS string) IS NULL OR ilike(e.name, CONCAT('%', CAST(:textSearch AS string), '%')) = true)")
    ...
    @Query("SELECT e FROM EntityViewEntity e WHERE e.tenantId = :tenantId " +
            ...
            "AND (:searchText IS NULL OR ilike(e.name, CONCAT('%', CAST(:searchText AS string), '%')) = true)")
    ...
    @Query("SELECT ev FROM EntityViewEntity ev, RelationEntity re WHERE ev.tenantId = :tenantId " +
            ...
            "AND (:searchText IS NULL OR ilike(ev.name, CONCAT('%', CAST(:searchText AS string), '%')) = true)")
